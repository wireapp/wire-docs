...
Typically, the deployment process runs seamlessly without requiring any external flags. However, if needed, you can skip certain tasks using their associated tags. For example, if you wish to skip the Enable Cert Manager hairpin Networking task—which is only required when using cert-manager with Let's Encrypt—you can use --skip-tags cert_manager_networking and configure your own certificates instead (see Bring your own certificates([./docs_ubuntu_22.04.md#bring-your-own-certificates](https://github.com/wireapp/wire-server-deploy/blob/master/offline/docs_ubuntu_22.04.md#bring-your-own-certificates)).
