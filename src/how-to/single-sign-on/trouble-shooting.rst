...
It is possible do delete a SAML connection directly via the API endpoint `DELETE /identity-providers/{id}`. However deleting a SAML connection also requires deleting all users that log in with this SAML connection. To prevent accidental deletion of users this functionality is not available directly from Team Settings.
