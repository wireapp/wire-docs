...
After retrieving the SRV record for a remote backend, the local backend should
cache the `backend domain <--> infra domain` mapping for the duration indicated
in the TTL field of the record.
